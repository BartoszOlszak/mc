...
public class TeamCommand implements CommandExecutor, TabCompleter {
        ...
        switch (subCommand) {
            ...
            case "create": {
                ...
                if (guildManager.getGuildByPlayer(player.getUniqueId()) != null) {
                    player.sendMessage(ChatColor.RED + "You are already in a team! You cannot create another.");
                    return true;
                }

                if (guildManager.isTooCloseToSpawn(player.getLocation())) {
                    player.sendMessage(ChatColor.RED + "You cannot create a guild this close to Spawn (0,0). You must be at least 500 blocks away.");
                    return true;
                }
                ...
                if (timeSinceDelete < oneHour) {
                    ...
                    player.sendMessage(ChatColor.RED + "You deleted a guild recently. You must wait " + minutesLeft + " minutes before creating a new one.");
                    return true;
                }

                if (guildManager.isTooCloseToOtherGuild(player.getLocation())) {
                    player.sendMessage(ChatColor.RED + "You are too close to another guild! Minimum distance is 200 blocks.");
                    return true;
                }
                ...
                if (guildManager.createGuild(tag, name, player.getUniqueId(), player.getLocation())) {
                    ...
                    player.sendMessage(ChatColor.GREEN + "Team " + name + " created! You are the Leader.");
            ...
            case "remove":
                ...
                if (targetToRemove.isOnline()) ((Player) targetToRemove).sendMessage(ChatColor.RED + "You were removed from team " + removeGuild.getName());
            ...
            case "delete": {
                ...
                if (age < oneHour && !player.isOp()) {
                    ...
                    player.sendMessage(ChatColor.RED + "Guild is too new! You must wait " + minutesLeft + " more minutes before deleting it.");
                    return true;
                }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommands = new ArrayList<>(Arrays.asList("help", "teams", "create", "add", "remove", "leave", "accept", "changeleader", "info", "delete", "advance", "degrade", "item"));
            if (sender.isOp()) subCommands.add("resettimers");
            ...
            } else if (Arrays.asList("accept", "info", "delete", "leave", "resettimers").contains(sub)) {
